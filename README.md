...
Data flows left to right: the **AviationStack API** is polled by **Airflow** (running in Docker on an EC2 instance), which lands raw JSON in **S3** and drives the **Snowflake** medallion layers (Bronze → Silver → Gold) via SQL, ending in a **Power BI** dashboard. Any task failure anywhere in the pipeline triggers a **Slack** alert.
...
### 3. Set up Snowflake
Run the scripts in `sql/` in order: `bronze_setup.sql` → the DAG handles `silver` and `gold` automatically on each run, but you can run them manually the first time from `silver_transform.sql` and `gold_aggregations.sql`.
